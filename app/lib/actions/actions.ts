...
export const getCustomer = async (customerId: string) => {
  const customer = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/customers/${customerId}`);
...
export const getOrders = async (customerId: string) => {
  const orders = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/order/customers/${customerId}`)
  console.log(orders)
  console.log((`${process.env.NEXT_PUBLIC_API_URL}/orders/customers/${customerId}`))
